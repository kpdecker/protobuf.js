...
tape.test(
  'by default, drop comments through de/serialization',
  async function (test) {
    test.plan(16);
...
tape.test(
  'preserve comments through de/serialization if option set',
  async function (test) {
    test.plan(8);
    try {
      const root = await protobuf.load(
      ...
      test.equal(
        root.lookup('TestMessage').comment,
        copy.lookup('TestMessage').comment
      );
      test.equal(
        root.lookup('TestMessage.testField').comment,
        copy.lookup('TestMessage.testField').comment
      );
      test.equal(
        root.lookup('TestMessage.testMap').comment,
        copy.lookup('TestMessage.testMap').comment
      );
      test.equal(
        root.lookup('TestMessage.testOneof').comment,
        copy.lookup('TestMessage.testOneof').comment
      );
      ...
      test.equal(
        rootService.methods['testMethod'].comment,
        copyService.methods['testMethod'].comment
      );
