...
tape.test('converters', async function (test) {
  const root = await protobuf.load('tests/data/convert.proto');
  // if (err)
  ...
  test.test(test.name + ' - Message#toObject', function (test) {
    ...
    test.test(test.name + ' - called with defaults = true', function (test) {
      ...
      test.equal(
        obj.enumVal,
        1,
        'should set enumVal to the first defined value'
      );
    ...
    test.test(
      test.name + ' - called with defaults = undefined',
      function (test) {
    ...
    test.test(test.name + ' - should convert', function (test) {
      ...
      test.same(
        Message.ctor.toObject(msg, { longs: Number }),
        msgLongsToNumber,
        'should convert the same using the static and the instance method'
      );
      test.same(
        Message.ctor.toObject(msg, { longs: String }),
        msgLongsToString,
        'should convert the same using the static and the instance method'
      );
      ...
      test.same(
        msgLongsToNumber.int64Map,
        { a: 2, b: 3 },
        'long map values to numbers'
      );
      test.same(
        msgLongsToString.int64Map,
        { a: '2', b: '3' },
        'long map values to strings'
      );

      test.equal(
        Object.prototype.toString.call(
          Message.toObject(msg, { bytes: Array }).bytesVal
        ),
        '[object Array]',
        'bytes to arrays'
      );
      test.equal(
        Message.toObject(msg, { bytes: String }).bytesVal,
        'MTEx',
        'bytes to base64 strings'
      );
      if (protobuf.util.isNode)
        test.ok(
          Buffer.isBuffer(Message.toObject(msg, { bytes: Buffer }).bytesVal),
          'bytes to buffers'
        );
  ...
  test.test(test.name + ' - Message.fromObject', function (test) {
    ...
    test.equal(
      ...
      msg,
      'should just return the object if already a runtime message'
    );
    ...
    test.same(
      msg.uint64Val,
      { low: 1, high: 0, unsigned: true },
      'should set uint64Val from a number'
    );
    test.same(
      msg.uint64Repeated,
      [
        { low: 1, high: 0, unsigned: true },
        { low: 2, high: 0, unsigned: true },
      ],
      'should set uint64Repeated from a number and a string'
    );
    test.same(msg.bytesVal, buf, 'should set bytesVal from a base64 string');
    test.same(
      msg.bytesRepeated,
      [buf, buf],
      'should set bytesRepeated from a base64 string and a plain array'
    );
    test.equal(msg.enumVal, 1, 'should set enumVal from a string');
    test.same(
      msg.enumRepeated,
      [2, 2],
      'should set enumRepeated from a number and a string'
    );
    test.same(
      msg.int64Map,
      {
        a: { low: 2, high: 0, unsigned: false },
        b: { low: 3, high: 0, unsigned: false },
      },
      'should set int64Map from a number and a string'
    );
    ...
    msg.$type = {
      toObject: function (obj, opt) {
        test.same(
          ...
          'should use toJSONOptions with toJSON'
        );
        test.end();
      },
    };
